...
in.dir <- c("SimRes_ests_converge_check")
boot.dir <- c("SimRes_boots2")
...
for(i in 1:n.cvs) {
  for(j in 1:n.runs) {
    ...
    perc.err.sd[i, , j] <- (mean.sd[i, , j] - true.sd[i, , j]) /
      true.sd[i, , j] * 100
    boot <- read_rds(paste0(boot.dir[j], "/cv", i, "_boot_res.rds"))
...
# Mean hessian sd
write.table(mean.sd, "Hessian standard deviations.txt")

# Sample sd
write.table(true.sd, "Sample standard deviations.txt")

# Bootstrap sd
write.table(boot.sd, "Bootstrap standard deviations.txt")

for (i in 1:6) {
  plot(true.sd[,i,1], type = "l",ylim = c(min(mean.sd[,i,1], true.sd[,i,1], boot.sd[,i,1]),
                                          max(true.sd[,i,1], true.sd[,i,1], boot.sd[,i,1])))
  lines(mean.sd[,i,1], type = "l", col = "blue")
  lines(boot.sd[,i,1], type = "l", col = "red")
}


plot(true.sd[,2,1], type = "l",ylim = c(min(mean.sd[,2,1]),max(true.sd[,2,1])))
lines(mean.sd[,2,1], type = "l", col = "blue")
lines(boot.sd[,2,1], type = "l", col = "red")
